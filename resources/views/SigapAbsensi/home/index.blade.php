  ...
  <div class="rounded-2xl border border-gray-200 bg-white p-3 shadow-sm">
    ...
    <div class="grid grid-cols-2 gap-2 mb-3">
      ...
      <button
        type="button"
        id="startGpsBtn"
        class="w-full px-4 py-3 rounded-xl border border-gray-300 text-sm font-semibold active:scale-[0.99]"
      >
        Aktifkan GPS
      </button>
    </div>
    ...
    <div class="grid grid-cols-2 gap-2 mt-3">
      <button
        id="captureBtn"
        type="button"
        class="w-full px-4 py-3 rounded-xl bg-maroon text-white text-sm font-semibold disabled:opacity-50 active:scale-[0.99]"
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function startCamera() {
    try {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        locationStatus.innerText = 'Browser tidak mendukung kamera.';
        return;
      }
      ...
      locationStatus.innerText = gpsReady
        ? 'Kamera aktif. GPS juga sudah aktif.'
        : 'Kamera aktif. Silakan aktifkan GPS.';
    } catch (err) {
      console.error(err);
      locationStatus.innerText = 'Kamera gagal dibuka. Cek izin kamera di browser.';
    }
  }
  ...
  function startGps() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        locationStatus.innerText = cameraReady
          ? 'Kamera aktif. GPS juga sudah aktif.'
          : 'GPS aktif. Silakan aktifkan kamera.';
      },
      (err) => {
        console.error(err);
        locationStatus.innerText = 'GPS tidak bisa diakses. Cek izin lokasi di browser.';
      },
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!cameraReady || !gpsReady) {
      e.preventDefault();
      alert('Aktifkan kamera dan GPS terlebih dahulu.');
      return;
    }
  });
});
