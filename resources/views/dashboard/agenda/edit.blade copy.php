      ...
      @foreach($agenda->items as $it)
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="mt-3 grid gap-3">
            ...
            <label class="block assignee-wrap">
              <span class="text-sm text-gray-800">Yang Ditugaskan</span>

              <div class="assignee-chips flex flex-wrap gap-1 mt-1"></div>

              <input type="text"
                    class="assignee-search mt-2 w-full rounded-lg border border-gray-300 p-2"
                    placeholder="Cari nama pegawai...">

              <div class="assignee-results border rounded mt-1 bg-white max-h-40 overflow-y-auto hidden"></div>

              <input type="text"
                    class="assignee-manual mt-2 w-full rounded-lg border border-gray-300 p-2"
                    placeholder="Tambah manual (Enter)">
...
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.assignee-wrap').forEach(initAssigneeEdit);
});
...
initAssigneeEdit(el.querySelector('.assignee-wrap'));
const srcAssignee = el.querySelector('.assignee-json');
const dstAssignee = dst.querySelector('.assignee-json');

if (srcAssignee && dstAssignee) {
  dstAssignee.value = srcAssignee.value;
  initAssigneeEdit(dst.querySelector('.assignee-wrap'));
}
...
function initAssigneeEdit(block){
  const hidden = block.querySelector('.assignee-json');
  if (!hidden.value) return;

  try {
    const data = JSON.parse(hidden.value);
    const state = { users: data.users || [], manual: data.manual || [] };

    block._assigneeState = state;
    renderAssigneeChips(block);
  } catch(e){}
}

function renderAssigneeChips(block){
  const chips = block.querySelector('.assignee-chips');
  const hidden = block.querySelector('.assignee-json');
  const state = block._assigneeState;

  chips.innerHTML = '';

  state.users.forEach((u, idx) => {
    const chip = document.createElement('span');
    chip.className = 'px-2 py-1 rounded-full bg-maroon text-white text-xs';
    chip.innerHTML = `${u.name} <button type="button">&times;</button>`;
  ...
  state.manual.forEach((m, idx) => {
    const chip = document.createElement('span');
    chip.className = 'px-2 py-1 rounded-full bg-gray-600 text-white text-xs';
    chip.innerHTML = `${m} <button type="button">&times;</button>`;
    chip.querySelector('button').onclick = () => {
