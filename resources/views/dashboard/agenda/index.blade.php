...
<section class="max-w-7xl mx-auto px-4 py-6">
  <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
    ...
    <div class="flex flex-wrap items-center gap-2">
      <!-- Tombol Menuju Halaman Pencarian Detail Kegiatan/Pegawai -->
      <a href="{{ route('sigap-agenda.search-items') }}"
         class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition shadow-sm text-sm font-medium">
      ...
      @hasrole('admin|verificator')
        <a href="{{ route('sigap-agenda.create') }}"
           class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-maroon text-white hover:bg-maroon-800 transition text-sm font-medium">
  ...
  <div class="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6">
    <form id="filterForm" class="flex flex-col sm:flex-row items-start sm:items-end gap-3" onsubmit="filterAgenda(event)">
      <div class="flex-1 w-full">
        <label class="block text-sm font-semibold text-gray-700">Filter Tanggal</label>
        <input id="filterDate" type="date"
               class="mt-1.5 w-full rounded-xl border border-gray-300 p-3 focus:border-maroon focus:ring-maroon text-sm">
      </div>
      <button type="submit" class="px-5 py-3 rounded-xl bg-maroon text-white hover:bg-maroon-800 text-sm font-medium">Terapkan Tanggal</button>
      <button type="button" onclick="resetFilter()" class="px-5 py-3 rounded-xl border border-gray-300 hover:bg-gray-50 text-sm font-medium">Bersihkan Tanggal</button>
    </form>
  </div>
</section>
...
function filterAgenda(e) {
  ...
  const filtered = AGENDA_DATA.filter(ag => {
    // Normalisasi format tanggal (YYYY-MM-DD)
    const agDate = (ag.date || '').substring(0, 10);
    return agDate === dateVal;
  });
