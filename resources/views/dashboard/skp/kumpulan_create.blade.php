...
<div x-data="kumpulanForm()" class="max-w-5xl mx-auto space-y-6">
  ...
  <form @submit.prevent="submitKumpulan" class="space-y-6">
    ...
    <div class="space-y-2">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[350px] overflow-y-auto p-2 border border-gray-200 rounded-2xl bg-white shadow-inner">
        @forelse($skpList as $skp)
          @php $foto = $skp->fotos->first(); @endphp
          <div @click="toggleSkp({{ $skp->id }})" class="relative rounded-xl border-2 p-2 cursor-pointer transition-all flex flex-col justify-between select-none" :class="selectedSkpIds.includes({{ $skp->id }}) ? 'border-maroon bg-maroon/5 ring-2 ring-maroon/20 shadow-md' : 'border-gray-200 hover:border-gray-300'">
            <div class="aspect-video w-full rounded-lg bg-gray-100 overflow-hidden mb-2 relative border border-gray-200">
              @if($foto) <img src="{{ asset('storage/' . $foto->file_path) }}" class="w-full h-full object-cover"> @else <div class="flex items-center justify-center h-full text-[10px] text-gray-400">Tidak ada gambar</div> @endif
              <div class="absolute top-1 right-1"><span x-show="selectedSkpIds.includes({{ $skp->id }})" class="w-5 h-5 rounded-full bg-maroon text-white flex items-center justify-center font-bold text-xs shadow-lg">✓</span></div>
            </div>
            <div>
              <p class="text-[10px] text-gray-500 font-semibold">📅 {{ \Carbon\Carbon::parse($skp->tanggal)->format('d/m/Y') }}</p>
              <h4 class="text-[11px] font-bold text-gray-800 line-clamp-2 leading-snug">{{ $skp->judul_kegiatan }}</h4>
            </div>
          </div>
        @empty
          <div class="col-span-full p-4 text-center text-xs text-gray-400 italic">Tidak ada laporan SKP di bulan ini.</div>
        @endforelse
      </div>
    </div>
    ...
    <div class="space-y-2">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-h-[350px] overflow-y-auto p-2 border border-gray-200 rounded-2xl bg-white shadow-inner">
        @forelse($ppdList as $ppd)
          @php 
            // Ambil foto pertama dari lembar milik user untuk thumbnail
            $firstLembar = $ppd->lembar->first();
            $fotoPpd = $firstLembar ? $firstLembar->fotos->first() : null;
          @endphp
          <div @click="togglePpd({{ $ppd->id }})" class="relative rounded-xl border-2 p-3 cursor-pointer transition-all flex flex-col justify-between select-none" :class="selectedPpdIds.includes({{ $ppd->id }}) ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-500/20 shadow-md' : 'border-gray-200 hover:border-gray-300'">
            <div class="flex gap-3">
              <div class="w-20 h-20 shrink-0 rounded-lg bg-gray-100 overflow-hidden relative border border-gray-200">
                @if($fotoPpd) <img src="{{ asset('storage/' . $fotoPpd->foto_path) }}" class="w-full h-full object-cover"> @else <div class="flex items-center justify-center h-full text-[9px] text-gray-400 text-center px-1">Tanpa Foto</div> @endif
                <div class="absolute top-1 right-1"><span x-show="selectedPpdIds.includes({{ $ppd->id }})" class="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-xs shadow-lg">✓</span></div>
              </div>
              <div class="flex-1">
                ...
                <p class="text-[10px] text-gray-500 font-medium mt-1">📍 {{ Str::limit($ppd->tempat, 20) }}</p>
                <p class="text-[10px] text-gray-500">📅 {{ $ppd->hari_tanggal }}</p>
              </div>
            </div>
          </div>
        @empty
          <div class="col-span-full p-4 text-center text-xs text-gray-400 italic">Tidak ada kegiatan PPD Anda di bulan ini.</div>
        @endforelse
      </div>
    </div>
