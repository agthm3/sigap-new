<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Buku Register Surat Masuk Tahun {{ $tahun }} — BRIDA Kota Makassar</title>
  
  <!-- Tailwind CSS CDN -->
  <script src="https://cdn.tailwindcss.com"></script>
  <script>
    tailwind.config = {
      theme: {
        extend: {
          colors: {
            maroon: {
              50:'#fdf7f7', 100:'#faeeee', 700:'#7a2222', 800:'#661b1b', DEFAULT:'#7a2222'
            }
          }
        }
      }
    }
  </script>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&family=Cinzel:wght@700&display=swap" rel="stylesheet">
  
  <style>
    body {
      font-family: 'Inter', sans-serif;
      background-color: #525659;
      margin: 0;
      padding: 20px 0;
      color: #111827;
    }

    @page {
      size: A4 landscape;
      margin: 10mm 10mm 15mm 10mm;
    }

    .book-page {
      background: white;
      width: 297mm;
      min-height: 210mm;
      margin: 0 auto 30px auto;
      padding: 15mm;
      box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.4);
      box-sizing: border-box;
      position: relative;
    }

    .page-break {
      page-break-after: always;
      break-after: page;
    }

    @media print {
      body {
        background: transparent !important;
        padding: 0 !important;
      }
      .no-print {
        display: none !important;
      }
      .book-page {
        width: 100% !important;
        min-height: 100% !important;
        margin: 0 !important;
        padding: 0 !important;
        box-shadow: none !important;
      }
    }
  </style>
</head>
<body>

  <!-- Floating Print & Filter Control Bar -->
  <div class="no-print fixed top-4 right-6 z-50 flex items-center gap-3 bg-white/95 backdrop-blur-md px-4 py-2 rounded-2xl shadow-xl border border-gray-200">
    <form method="GET" action="{{ route('sigap-surat.masuk.buku-agenda') }}" class="flex items-center gap-1.5 text-xs">
      <span class="font-bold text-gray-700">Tahun:</span>
      <select name="tahun" onchange="this.form.submit()" class="rounded-lg border border-gray-300 py-1 px-2 text-xs font-semibold bg-gray-50 focus:ring-0">
        @for($y = date('Y'); $y >= 2024; $y--)
          <option value="{{ $y }}" {{ $tahun == $y ? 'selected' : '' }}>{{ $y }}</option>
        @endfor
      </select>
    </form>

    <span class="text-gray-300">|</span>

    <button onclick="window.print()" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-maroon text-white text-xs font-bold hover:bg-maroon-800 shadow transition">
      🖨️ Cetak / PDF
    </button>

    @auth
      <a href="{{ route('sigap-surat.masuk.index') }}" class="px-3 py-1.5 rounded-xl border border-gray-300 text-xs font-semibold text-gray-700 hover:bg-gray-100">
        Ke Dashboard
      </a>
    @endauth
  </div>

  <!-- ========================================================================= -->
  <!-- HALAMAN 1: SAMPUL DEPAN (COVER BUKU REGISTER SURAT MASUK) -->
  <!-- ========================================================================= -->
  <div class="book-page page-break flex flex-col justify-between items-center text-center border-[6px] border-double border-gray-900 p-16">
    
    <!-- Kop Sampul -->
    <div class="pt-8 space-y-2">
      <h3 class="text-lg font-bold tracking-widest uppercase text-gray-700">Pemerintah Kota Makassar</h3>
      <h2 class="text-2xl font-black tracking-wider uppercase text-gray-900">Badan Riset dan Inovasi Daerah</h2>
      <div class="w-32 h-1 bg-maroon mx-auto mt-2"></div>
    </div>

    <!-- Judul Utama -->
    <div class="my-auto space-y-6">
      <div class="inline-block p-6 border-2 border-gray-800 rounded-2xl">
        <h1 class="text-4xl font-black uppercase tracking-widest text-gray-900" style="font-family: 'Cinzel', serif;">
          Buku Register Agenda
        </h1>
        <p class="text-2xl font-extrabold text-maroon tracking-widest uppercase mt-2">
          Surat Masuk
        </p>
      </div>

      <div class="text-sm font-semibold text-gray-600 tracking-wide">
        Tata Naskah Dinas & Penerimaan Surat Masuk
      </div>

      <div class="text-6xl font-black text-gray-900 tracking-wider pt-2">
        TAHUN {{ $tahun }}
      </div>
    </div>

    <!-- Footer Sampul -->
    <div class="w-full flex items-center justify-between border-t border-gray-300 pt-4 px-4 text-xs text-gray-500">
      <span>Sistem Informasi Tata Naskah Dinas & Persuratan</span>
      <span class="font-bold text-gray-700">SIGAP SURAT — BRIDA KOTA MAKASSAR</span>
    </div>

  </div>

  <!-- ========================================================================= -->
  <!-- HALAMAN 2 DST: ISI TABEL REGISTER AGENDA SURAT MASUK -->
  <!-- ========================================================================= -->
  <div class="book-page">
    
    <!-- Kop Halaman -->
    <div class="flex items-center justify-between border-b-2 border-gray-900 pb-3 mb-4">
      <div>
        <h2 class="text-base font-black uppercase text-gray-900">Buku Register Agenda Surat Masuk</h2>
        <p class="text-xs text-gray-600">Badan Riset dan Inovasi Daerah Kota Makassar — Tahun Takwim {{ $tahun }}</p>
      </div>
      <div class="text-right text-xs">
        <span class="px-2.5 py-1 rounded bg-gray-100 font-bold text-gray-700">KODE OPD: BRIDA</span>
      </div>
    </div>

    <!-- Tabel Register -->
    <table class="w-full text-xs border-collapse border border-gray-900">
      <thead>
        <tr class="bg-gray-100 text-gray-900 text-center font-bold uppercase">
          <th class="border border-gray-900 px-2 py-2.5 w-14">No. Agenda</th>
          <th class="border border-gray-900 px-2 py-2.5 w-24">Tgl Terima</th>
          <th class="border border-gray-900 px-3 py-2.5 w-48">Asal Surat / Pengirim</th>
          <th class="border border-gray-900 px-3 py-2.5 w-44">Nomor & Tgl Surat</th>
          <th class="border border-gray-900 px-3 py-2.5">Perihal / Isi Ringkas Surat</th>
          <th class="border border-gray-900 px-2 py-2.5 w-20">Sifat</th>
          <th class="border border-gray-900 px-3 py-2.5 w-32">Unit Pengolah</th>
          <th class="border border-gray-900 px-2 py-2.5 w-32">Penerima & TTD</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-gray-900">
        @forelse($surats as $item)
          <tr>
            <td class="border border-gray-900 px-2 py-2 text-center font-bold text-gray-900 font-mono">
              {{ sprintf('%03d', $item->nomor_agenda) }}
            </td>
            <td class="border border-gray-900 px-2 py-2 text-center whitespace-nowrap">
              {{ $item->tanggal_terima->format('d/m/Y') }}
            </td>
            <td class="border border-gray-900 px-3 py-2 font-semibold text-gray-900">
              {{ $item->asal_surat }}
            </td>
            <td class="border border-gray-900 px-3 py-2 font-mono text-[11px]">
              <div>{{ $item->nomor_surat_masuk }}</div>
              <div class="text-[10px] text-gray-500 font-sans">Tgl: {{ $item->tanggal_surat->format('d/m/Y') }}</div>
            </td>
            <td class="border border-gray-900 px-3 py-2 leading-relaxed">
              {{ $item->perihal }}
            </td>
            <td class="border border-gray-900 px-2 py-2 text-center text-[11px]">
              {{ $item->tingkat_surat }}
            </td>
            <td class="border border-gray-900 px-3 py-2 text-[11px] text-gray-700">
              {{ $item->unit_pengolah }}
            </td>
            <td class="border border-gray-900 px-2 py-2 text-center">
              <div class="flex flex-col items-center justify-center">
                @if($item->ttd_penerima)
                  <img src="{{ $item->ttd_penerima }}" alt="TTD" class="h-8 max-w-[80px] object-contain mb-0.5">
                @endif
                <span class="text-[10px] font-bold text-gray-800 uppercase">{{ $item->penerima->name ?? '-' }}</span>
              </div>
            </td>
          </tr>
        @empty
          <tr>
            <td colspan="8" class="border border-gray-900 px-4 py-8 text-center text-gray-500 italic">
              Tidak ada data buku register surat masuk yang tercatat untuk tahun {{ $tahun }}.
            </td>
          </tr>
        @endforelse
      </tbody>
    </table>

    <!-- Footer Verifikasi Resmi -->
    <div class="mt-8 pt-4 border-t border-dashed border-gray-400 flex items-center justify-between text-[10px] text-gray-600">
      <div class="flex items-center gap-2">
        <div class="w-6 h-6 rounded bg-maroon text-white flex items-center justify-center font-bold text-xs">
          ✓
        </div>
        <div>
          <p class="font-bold text-gray-800">DIVERIFIKASI DAN DICETAK SECARA RESMI OLEH SIGAP SURAT</p>
          <p>Badan Riset dan Inovasi Daerah (BRIDA) Kota Makassar • Dokumen ini sah dan diakui secara digital.</p>
        </div>
      </div>
      <div class="text-right">
        <p>Waktu Cetak: {{ now()->translatedFormat('d F Y, H:i') }} WITA</p>
        <p class="font-mono text-gray-400">UUID: {{ strtoupper(substr(md5('SURAT_MASUK_' . $tahun . now()), 0, 16)) }}</p>
      </div>
    </div>

  </div>

</body>
</html>