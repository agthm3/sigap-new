...
Route::middleware(['auth'])->group(function () {
    Route::prefix('sigap-daftar-hadir')->name('sigap-daftar-hadir.')->group(function () {
        ...
        // ← ROUTE STATIS BARU — harus di atas /{kegiatan}
        Route::get('/riwayat-peserta',
            [SigapDaftarHadirController::class, 'riwayatPeserta'])
            ->name('riwayat-peserta');
 
        Route::get('/riwayat-peserta/detail',
            [SigapDaftarHadirController::class, 'riwayatPesertaDetail'])
            ->name('riwayat-peserta.detail');
 
        // Route wildcard — tetap di bawah
