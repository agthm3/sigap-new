...
<div class="mb-6">
  ...
  <p class="text-sm text-gray-600 mt-1">
    Sub-Kegiatan Induk: <span class="font-bold text-maroon whitespace-pre-line">{{ $subKegiatan->nama_sub_kegiatan }}</span>
  </p>
</div>
    ...
    <div class="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
      ...
      <form action="{{ route('sigap-spj.bidang.kegiatan.store', $subKegiatan->id) }}" method="POST">
        ...
        <div class="mb-4">
          ...
          <textarea name="nama_kegiatan" required rows="3" placeholder="Contoh: Rapat Koordinasi dengan OPD Terkait" 
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-gray-50 text-xs uppercase text-gray-600">
            <tr>
              ...
              <th class="px-4 py-3 text-center">Jml Gelombang</th>
          ...
          <tbody class="divide-y divide-gray-100">
            @forelse($subKegiatan->kegiatans as $keg)
              <tr>
                <td class="px-4 py-3 font-medium text-gray-900 whitespace-pre-line">{{ $keg->nama_kegiatan }}</td>
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <form action="{{ route('sigap-spj.bidang.kegiatan.destroy', $keg->id) }}" method="POST" class="inline">
                      @csrf
                      @method('DELETE')
                      <button type="button" class="btn-delete px-3 py-1.5 rounded border border-red-500 text-red-600 text-xs font-medium hover:bg-red-600 hover:text-white" data-judul="{{ $keg->nama_kegiatan }}">
                        Hapus
                      </button>
                    </form>
                  </div>
  ...
  document.querySelectorAll('.btn-delete').forEach(function(btn) {
    btn.addEventListener('click', function () {
      ...
      Swal.fire({
        ...
        html: `Kegiatan ini beserta data gelombang di dalamnya akan dihapus permanen!`,
        icon: 'warning',
