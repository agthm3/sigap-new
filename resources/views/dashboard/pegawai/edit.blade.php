  ...
  <main class="max-w-7xl mx-auto px-4 py-6 grid lg:grid-cols-3 gap-6">
    ...
    <section class="lg:col-span-2 space-y-6">
      <form id="fPegawai" method="POST"
            action="{{ route('sigap-pegawai.update', $employee->id) }}"
        ...
        <div class="grid sm:grid-cols-2 gap-4">
          ...
          <label class="block">
            ...
            <select name="unit" required
              ...
              @foreach ($units as $u)
                <option value="{{ $u }}" @selected(old('unit', $employee->unit)===$u)>{{ $u }}</option>
              @endforeach
            </select>
          </label>
        </div>
    ...
    <aside class="space-y-6">
      <div class="bg-white border border-gray-200 rounded-2xl p-5">
        ...
        <div class="mt-3 flex items-center gap-4">
          @php
            $avatar = $employee->avatar_path ? asset('storage/'.$employee->avatar_path) : null;
            $initial = collect(explode(' ', $employee->name))->map(fn($w)=>mb_substr($w,0,1))->take(2)->implode('');
          @endphp
          ...
          <div class="flex-1">
            <form id="avatarForm" method="POST" action="{{ route('sigap-pegawai.update', $employee->id) }}" enctype="multipart/form-data" class="space-y-2">
              @csrf
              @method('PUT')
      ...
      {{-- <div class="bg-white border border-gray-200 rounded-2xl p-5">
        ...
        <div class="mt-3 flex flex-col gap-2">
          <a href="{{ route('employee-documents.create', $employee->id) }}"
